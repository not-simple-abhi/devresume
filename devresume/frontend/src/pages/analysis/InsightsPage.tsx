...
function ReadinessBanner({ status }: { status: string }) {
  const config = {
    ...
    needs_work: {
      ...
      desc: "Based on standard recruiter screening patterns, your resume lacks quantifiable metrics in the most recent role.",
    },
    not_ready: {
      icon: Flag,
      label: 'Not Ready for Interviews',
      bg:   'bg-red-50 dark:bg-red-950/50 border-red-300 dark:border-red-800',
      text: 'text-red-700 dark:text-red-400',
  ...
  return (
    <div className={cn('flex items-start gap-3 border rounded-xl px-4 py-4', c.bg)}>
      ...
      <div>
        ...
        <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mt-1 leading-relaxed">{c.desc}</p>
      </div>
    </div>
  )
}
    ...
    <div className="space-y-4 pt-4 animate-fade-in">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <Card>
          ...
          {recruiter.strengths.length > 0 ? (
            <ul className="space-y-3">
              {recruiter.strengths.map((s, i) => (
                <li key={i} className="flex items-start gap-2.5">
                  <span className="text-gray-300 dark:text-gray-600 text-sm shrink-0 mt-0.5 font-mono">›</span>
                  <div>
                    <p className="text-sm font-bold text-gray-800 dark:text-gray-200">{s.split(/[.,:]/)[0]}</p>
                    {s.includes('.') && (
                      <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mt-0.5">{s.split(/[.:]/)[1]?.trim()}</p>
                    )}
                  </div>
                </li>
              ))}
            ...
            <p className="text-sm font-medium text-gray-400 dark:text-gray-500">No strengths identified.</p>
          )}
        </Card>
        ...
        <Card>
          <div className="flex items-center gap-2 mb-3 pb-2.5 border-b border-gray-100 dark:border-gray-800">
            ...
            <h3 className="text-base font-semibold text-gray-800 dark:text-gray-200">Areas for Improvement</h3>
          </div>
          {recruiter.weaknesses.length > 0 ? (
            <ul className="space-y-3">
              {recruiter.weaknesses.map((w, i) => (
                <li key={i} className="flex items-start gap-2.5">
                  <span className="text-gray-300 dark:text-gray-600 text-sm shrink-0 mt-0.5 font-mono">›</span>
                  <div>
                    <p className="text-sm font-bold text-gray-800 dark:text-gray-200">{w.split(/[.,:]/)[0]}</p>
                    {w.includes('.') && (
                      <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mt-0.5">{w.split(/[.:]/)[1]?.trim()}</p>
                    )}
                  </div>
                </li>
              ))}
            ...
            <p className="text-sm font-medium text-gray-400 dark:text-gray-500">No areas for improvement identified.</p>
          )}
        </Card>
      </div>
      ...
      {recruiter.standoutPoints.length > 0 && (
        <Card>
          <div className="flex items-center gap-2 mb-3 pb-2.5 border-b border-gray-100 dark:border-gray-800">
            ...
            <h3 className="text-base font-semibold text-gray-800 dark:text-gray-200">Standout Points</h3>
          </div>
      ...
      {recruiter.redFlags.length > 0 && (
        <Card className="border-red-100 dark:border-red-900">
          <div className="flex items-center gap-2 mb-3 pb-2.5 border-b border-red-50 dark:border-red-900">
            <Flag size={15} className="text-red-400" />
            <h3 className="text-base font-semibold text-gray-800 dark:text-gray-200">Red Flags</h3>
            <Badge variant="red" size="sm">{recruiter.redFlags.length}</Badge>
          </div>
          <ul className="space-y-2.5">
            {recruiter.redFlags.map((f, i) => (
              <li key={i} className="flex items-start gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-red-50 dark:bg-red-950/50 rounded-lg px-3 py-2.5">
  ...
  return (
    <div className="space-y-4 pt-4 animate-fade-in">
      <div className="flex flex-wrap gap-3">
        <div className="flex items-center gap-2 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl px-4 py-2.5 shadow-sm">
          <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Writing Quality</span>
          <Badge variant="default" className={cn('capitalize font-semibold', qualityColor[grammar.writingQuality ?? 'average'])}>
        ...
        <div className="flex items-center gap-2 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl px-4 py-2.5 shadow-sm">
          <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Tense Consistent</span>
      ...
      {grammar.errors.length > 0 && (
        <Card>
          <div className="flex items-center gap-2 mb-4 pb-3 border-b border-gray-100 dark:border-gray-800">
            <AlertTriangle size={15} className="text-red-400" />
            <h3 className="text-base font-semibold text-gray-800 dark:text-gray-200">Grammar Errors</h3>
          ...
          <div className="space-y-3">
            {grammar.errors.map((err, i) => {
              ...
              return (
                <div key={i} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 border border-gray-100 dark:border-gray-700">
                  {original   && <p className="text-sm font-medium text-red-500 line-through mb-1.5">{original}</p>}
                  {suggestion && <p className="text-sm font-medium text-gray-700 dark:text-gray-300">{suggestion}</p>}
                  {!original && !suggestion && <p className="text-sm font-medium text-gray-600 dark:text-gray-400">{JSON.stringify(err)}</p>}
                </div>
              )
            })}
          </div>
      ...
      {grammar.errors.length === 0 && grammar.suggestions.length === 0 && (
        <div className="bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-800 rounded-xl p-6 text-center">
          <CheckCircle2 size={28} className="text-emerald-400 mx-auto mb-2" />
          <p className="text-base font-bold text-emerald-700 dark:text-emerald-400">No grammar issues found</p>
          <p className="text-sm font-medium text-emerald-600 dark:text-emerald-500 mt-1">Your writing is clean and professional.</p>
        </div>
      )}
    </div>
  )
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">
              ...
              <span className="text-gray-700 dark:text-gray-300 font-semibold">'Senior Frontend Engineer'</span> role.
            </p>
          </div>
        </div>
        <Button variant="secondary" size="sm" icon={<RefreshCw size={13} />}>Re-run Analysis</Button>
      </div>
