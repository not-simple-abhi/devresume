...
function HeroMockup() {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-xl shadow-violet-100/40 p-5 w-full max-w-xs">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-xs font-semibold text-gray-800">Analysis Report</p>
          <p className="text-[10px] text-gray-400">john_doe_backend.pdf</p>
        </div>
        <span className="text-[9px] px-2 py-0.5 bg-amber-50 text-amber-600 border border-amber-200 rounded-full font-medium">
          ★ REVIEWED
        </span>
      </div>
        ...
        {bars.map((b) => (
          <div key={b.label}>
            <div className="flex justify-between mb-1">
              ...
              <span className="text-[10px] font-mono font-semibold" style={{ color: b.color }}>
                {b.pct}%
              </span>
            </div>
...
const pipelineSteps = [
  {
    ...
    desc: 'Parse deep persons to ensure ATS systems can read your job metadata and experience timeline.',
    icon: Code2,
  ...
  },
  {
    ...
    desc: "Map your titles' technologies against a library of 30,000+ developer tools and frameworks.",
    icon: Tag,
    color: 'bg-blue-50 text-blue-600',
  },
  {
    step: '03 / EVALUATE',
    title: 'Contextual Impact',
    desc: 'LLMs evaluate bullet points for technical depth, architectural understanding, and business impact.',
    icon: Zap,
  ...
  },
  {
    ...
    desc: 'Generate technically sound, metric-focused rewrites that sound like an engineer, not a marketer.',
    icon: RefreshCw,
...
const capabilities = [
  {
    icon: ScanText,
    title: 'Deep Syntax Highlighting',
    desc: "We don't just read words, we understand context. See exactly which phrases are strong action verbs and which are passive filler.",
    cta: 'Explore Feature',
    featured: false,
  },
  {
    icon: Brain,
    title: 'The "Seniority Scanner"',
    desc: 'Does your resume sound junior? Our models detect language patterns associated with different seniority levels, helping you project authority and architectural leadership.',
    featured: true,
    badge: '+ AI LAYER',
  },
  {
    icon: ScanText,
    title: 'Live ATS Parsing',
    desc: 'See exactly how Workday or Greenhouse will extract your data before you hit submit.',
    featured: false,
  },
  {
    icon: RefreshCw,
    title: 'Job Description Diff',
    desc: 'Paste a JD and generate a missing keyword matrix instantly.',
    featured: false,
  },
]
...
// ─── Feature cards in capabilities grid ──────────────────────────────────────
function CapabilityCard({
  ...
  if (featured) {
    return (
      <div className="row-span-2 bg-violet-600 rounded-2xl p-6 flex flex-col text-white relative overflow-hidden">
        {/* bg decoration */}
        ...
        {badge && (
          <span className="self-end text-[9px] px-2 py-0.5 bg-white/20 text-white rounded-full mb-4 font-medium relative z-10">
            {badge}
          </span>
        )}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md hover:border-violet-100 transition-all">
      ...
      {cta && (
        <button className="mt-3 text-xs text-violet-600 font-medium hover:text-violet-700 flex items-center gap-1">
  ...
  return (
    <div className="landing-bg min-h-screen">
      ...
      <section id="hero" className="max-w-7xl mx-auto px-4 sm:px-6 pt-16 pb-20 grid lg:grid-cols-2 gap-12 items-center scroll-mt-14">
        {/* Left */}
        <div>
          <div className="inline-flex items-center gap-1.5 text-xs font-medium text-violet-600 bg-violet-50 border border-violet-200 rounded-full px-3 py-1 mb-5">
            <Sparkles size={11} />
          ...
          <h1 className="text-4xl sm:text-5xl font-bold text-gray-900 leading-tight mb-4">
            Your Resume,<br />
            Analyzed Like an{' '}
            <span className="text-violet-600">Engineer</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            <Link
              to="/analyze"
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-violet-600 text-white text-sm font-medium rounded-xl hover:bg-violet-700 transition-colors shadow-md shadow-violet-200"
            >
              <Upload size={15} />
              Upload Your Resume — It's Free
            </Link>
            <button className="inline-flex items-center gap-2 px-5 py-2.5 border border-gray-200 text-gray-700 text-sm font-medium rounded-xl hover:bg-gray-50 transition-colors">
      ...
      <section id="pipeline" className="bg-white/60 backdrop-blur-sm border-t border-b border-violet-100/50 py-20 scroll-mt-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {pipelineSteps.map(({ step, title, desc, icon: Icon, color }) => (
              <div
                key={step}
                className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md hover:border-violet-100 transition-all animate-slide-up"
              >
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center mb-3 ${color}`}>
                  <Icon size={16} />
                </div>
                <p className="text-[9px] font-mono font-bold text-gray-300 mb-1 tracking-widest">
                  {step}
                </p>
      ...
      <section id="capabilities" className="max-w-7xl mx-auto px-4 sm:px-6 py-20 scroll-mt-14">
        <div className="text-center mb-12">
          <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-3">Core Capabilities</h2>
        </div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 auto-rows-auto">
          {/* First 2 regular cards */}
          <CapabilityCard {...capabilities[0]} />
          {/* Featured card spans rows */}
      ...
      {/* ── Company Readiness ── */}
      <section className="bg-white/60 backdrop-blur-sm border-t border-violet-100/50 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-3">
          ...
          <div className="mt-10 text-center">
            <Link
              to="/company"
              className="inline-flex items-center gap-2 text-sm text-violet-600 font-medium hover:text-violet-700"
            >
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-20">
        <div className="bg-violet-600 rounded-3xl p-10 sm:p-14 text-center relative overflow-hidden">
          <div className="absolute inset-0 opacity-20"
            style={{
              backgroundImage: 'radial-gradient(circle at 20% 50%, #a78bfa 0%, transparent 50%), radial-gradient(circle at 80% 50%, #818cf8 0%, transparent 50%)',
            }}
          />
      ...
      <footer className="border-t border-gray-100 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-gray-700">DevResume</span>
          </div>
